...
{       
    ...
    limit = 3;//MPI_Allreduce(&sum_loc, &limit, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allgather(pp, nsv/s, MPI_DOUBLE, pf, nsv/s, MPI_DOUBLE, MPI_COMM_WORLD);
    
    while(abs(limit) >0.0001){
         //for (int i=0; i<N; i++){
            //     cout<< rf[i]<< " ";
              //}
             //cout << endl;
         //F77NAME(dgemv)('N', N, N, 1, A, N, pf, 1, 0, ap, 1);
         for (int i=0; i< nsv; ++i){
             ap[i] = 0;
             for (int j=0; j< nsv; ++j){
